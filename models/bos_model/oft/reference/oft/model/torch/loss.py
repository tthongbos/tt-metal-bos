...
def hard_neg_mining_loss(scores, labels, neg_ratio=5):
    ...
    return F.binary_cross_entropy_with_logits(scores, labels.float(), mask, reduction="sum")
...
def compute_uncertainty(logvar, sqr_dists, min_dist):
    ...
    return min_dist / torch.sqrt(var) * torch.exp(-0.5 * (sqr_dists / logvar - 1.0))
...
def log_ap_loss(logvar, sqr_dists, num_thresh=10):
    ...
    thresholds = torch.logspace(math.log10(1 / maxvar), math.log10(max_dist / minvar), num_thresh).type_as(stdev)
    ...
    loss = (loss[0] + loss[-1]) / 2.0 + loss[1:-1].sum()
