...
from model_dev.oft_model.tt.operations import Conv, GroupNormDram, GroupNormL1, Relu
...
class TTBasicBlock:
    ...
    def forward(self, device, x, shard="HS", *, num_splits=1, collect_intermediates=True):
        outs = {}
        identity = x

        # logger.info(f"Block id {self.block_id}: conv1 start")
        out, _, _ = self.conv1(device, x, shard)
        # out = ttnn.move(out)
        ...
        out, _, _ = self.conv2(device, out, shard)
        # logger.info(f"Block id {self.block_id} with output in: {out.memory_config()} layout: {out.layout} shape: {out.shape}")
        # out = ttnn.move(out)
        if collect_intermediates:
...
class TTResNet:
    def __init__(self, device, parameters, layer_args, *, dtype=ttnn.bfloat8_b):
        self.conv1 = Conv(parameters.conv1, layer_args.conv1, weight_dtype=dtype)
        # self.bn1 = GroupNormL1(parameters.bn1, layer_args.bn1, dtype=dtype)
        self.bn1 = GroupNormDram(parameters.bn1, layer_args.bn1, dtype=dtype)
        self.relu = Relu()
    ...
    @staticmethod
    def _run_layer(layer, device, x, shard, *, num_splits=1, collect_intermediates=False):
        layer_outs = {}
    ...
    def forward_feature_pyramid(self, device, x, shard, *, num_splits=1, collect_intermediates=False):
        outs = {}
        ...
        x, _, _ = self.conv1(device, x, shard)
        ...
        # x = self.bn1(device, shard = "HS", input_tensor=x, num_splits=num_splits)
