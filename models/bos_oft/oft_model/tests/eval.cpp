...
=======================================================================*/
vector<tDetection> loadDetections(
    string file_name,
    bool& compute_aos,
    vector<bool>& eval_image,
    vector<bool>& eval_ground,
    vector<bool>& eval_3d,
    bool& success) {
        ...
        if (fscanf(
            ...
            for (int c = 0; c < NUM_CLASS; c++) {
                if (!strcasecmp(d.box.type.c_str(), CLASS_NAMES[c].c_str()) ||
                    ...
                    if (!eval_3d[c] && d.t1 != -1000 && d.t2 != -1000 && d.t3 != -1000 && d.h > 0 && d.w > 0 &&
                        d.l > 0) {
                        eval_3d[c] = true;
                    }
                    break;
...
vector<tGroundtruth> loadGroundtruth(string file_name, bool& success) {
    ...
    while (!feof(fp)) {
        ...
        if (fscanf(
                fp,
                "%s %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
                str,
                &g.truncation,
                &g.occlusion,
                &g.box.alpha,
...
void cleanData(
    ...
    for (int32_t i = 0; i < gt.size(); i++) {
        ...
        // classes with a neighboring class
            ...
            !strcasecmp(CLASS_NAMES[current_class].c_str(), "Pedestrian") &&
            !strcasecmp("Person_sitting", gt[i].box.type.c_str())) {
            valid_class = 0;
        } else if (
            !strcasecmp(CLASS_NAMES[current_class].c_str(), "Car") && !strcasecmp("Van", gt[i].box.type.c_str())) {
            valid_class = 0;
        }
...
tPrData computeStatistics(
    ...
    for (int32_t i = 0; i < gt.size(); i++) {
        ...
        for (int32_t j = 0; j < det.size(); j++) {
            ...
            // if the greatest overlap is an ignored detection (min_height), the overlapping detection is used
                ...
                compute_fp && overlap > MIN_OVERLAP[metric][current_class] &&
                ...
                compute_fp && overlap > MIN_OVERLAP[metric][current_class] && valid_detection == NO_DETECTION &&
                ignored_det[j] == 1) {
                det_idx = j;
                valid_detection = 1;
    ...
    if (compute_fp) {
        ...
        for (int32_t i = 0; i < dc.size(); i++) {
            for (int32_t j = 0; j < det.size(); j++) {
                // detections not of the current class, already assigned, with a low threshold or a low minimum height
                // are ignored
                if (assigned_detection[j]) {
                    continue;
                }
...
int32_t main(int32_t argc, char* argv[]) {
    ...
    if (eval(result_sha, mail)) {
        ...
        mail->msg(
            "http://www.cvlibs.net/datasets/kitti/user_submit_check_login.php?benchmark=object&user=%s&result=%s",
            argv[2],
            result_sha.c_str());
    } else {
