...
class ObjectEncoder(nn.Module):
    def __init__(
        ...
        kernel = gaussian_kernel(sigma, dtype=dtype).expand(num_classes, num_classes, -1, -1)
    ...
    def decode(self, heatmaps, pos_offsets, dim_offsets, ang_offsets, grid):
        peaks, max_inds, scores, classids, smoothed, max_pooled = self._decode_heatmaps(heatmaps)
        ...
        return [scores, classids, positions, dimensions, angles], [peaks, max_inds, smoothed, max_pooled]

    def create_objects(self, scores, classids, positions, dimensions, angles):
        objects = []
        for score, classid, position, dimension, angle in zip(scores, classids, positions, dimensions, angles):
            objects.append(ObjectData(self.classnames[int(classid)], position, dimension, angle, score))
        return objects

    def forward(self, heatmaps, pos_offsets, dim_offsets, ang_offsets, grid):
        decoded, _ = self.decode(heatmaps, pos_offsets, dim_offsets, ang_offsets, grid)
        return self.create_objects(*decoded)

    def _decode_heatmaps(self, heatmaps):
        peaks, max_inds, smoothed, max_pooled = self._non_maximum_suppression(heatmaps)
        scores = heatmaps[peaks]
        classids = torch.nonzero(peaks)[:, 0]
        return peaks, max_inds, scores, classids, smoothed, max_pooled
    ...
    def _non_maximum_suppression(self, heatmaps):
        smoothed = self.nms_conv(heatmaps[None])
        max_pooled, max_inds = F.max_pool2d(smoothed, 3, stride=1, padding=1, return_indices=True)
        max_inds = max_inds.squeeze(0)

        _, height, width = heatmaps.size()
        flat_inds = torch.arange(height * width, device=heatmaps.device).type_as(max_inds).view(height, width)
        ...
        if peaks.long().sum() > self.max_peaks:
            ...
            peaks = peaks & (heatmaps > scores[self.max_peaks - 1])

        return peaks, max_inds, smoothed, max_pooled
